...
void CInsetWindow::setAirport(string icao) { this->icao = icao; }

void CInsetWindow::OnClickScreenObject(const char *sItemString, POINT Pt,
                                       int Button) {
...
bool CInsetWindow::OnMoveScreenObject(const char *sObjectId, POINT Pt,
                                      RECT Area, bool Released) {
  if (strcmp(sObjectId, "window") == 0) {
    ...
    POINT maxoffset = {(m_Area.right - m_Area.left) / 2,
                       (m_Area.bottom - (m_Area.top + 15)) / 2};
    m_Offset.x =
        max(-maxoffset.x,
            min(maxoffset.x, m_OffsetInit.x + (Pt.x - m_OffsetDrag.x)));
    m_Offset.y =
        max(-maxoffset.y,
            min(maxoffset.y, m_OffsetInit.y + (Pt.y - m_OffsetDrag.y)));
  ...
  if (strcmp(sObjectId, "topbar") == 0) {
    ...
    POINT BottomRight = {TopLeft.x + appWindowRect.Width(),
                         TopLeft.y + appWindowRect.Height()};
...
void CInsetWindow::render(HDC hDC, CSMRRadar *radar_screen, Graphics *gdi,
                          POINT mouseLocation,
  ...
  struct Utils {
    ...
    static RECT drawToolbarButton(CDC *dc, string letter, CRect TopBar,
                                  int left, POINT mouseLocation) {
  ...
  COLORREF qBackgroundColor = radar_screen->CurrentConfig->getConfigColorRef(
      radar_screen->CurrentConfig
          ->getActiveProfile()["approach_insets"]["background_color"]);
  ...
  CSectorElement rwy;
  for (rwy = radar_screen->GetPlugIn()->SectorFileElementSelectFirst(
           SECTOR_ELEMENT_RUNWAY);
       rwy.IsValid();
       rwy = radar_screen->GetPlugIn()->SectorFileElementSelectNext(
    ...
    if (startsWith(icao.c_str(), rwy.GetAirportName())) {
      ...
      CPen RunwayPen(
          PS_SOLID, 1,
          radar_screen->CurrentConfig->getConfigColorRef(
              radar_screen->CurrentConfig
                  ->getActiveProfile()["approach_insets"]["runway_color"]));
      CPen ExtendedCentreLinePen(
          PS_SOLID, 1,
          radar_screen->CurrentConfig->getConfigColorRef(
              radar_screen->CurrentConfig
                  ->getActiveProfile()["approach_insets"]
      ...
      if (rwy.IsElementActive(false, 0) || rwy.IsElementActive(false, 1)) {
        ...
        double lenght = double(radar_screen->CurrentConfig
                                   ->getActiveProfile()["approach_insets"]
        ...
        CPosition endExtended =
            BetterHarversine(Threshold, reverseHeading, lenght);
        ...
        // Drawing the ticks
        int increment = radar_screen->CurrentConfig
                            ->getActiveProfile()["approach_insets"]
        ...
        for (int j = increment;
             j <= int(radar_screen->CurrentConfig
                          ->getActiveProfile()["approach_insets"]
                                              ["extended_lines_length"]
                          .GetInt() *
                      1852);
          ...
          CPosition tickPosition =
              BetterHarversine(Threshold, reverseHeading, j);
          CPosition tickBottom = BetterHarversine(
              tickPosition, fmod(reverseHeading - 90, 360), 500);
          CPosition tickTop = BetterHarversine(
              tickPosition, fmod(reverseHeading + 90, 360), 500);
  ...
  vector<POINT> appAreaVect = {windowAreaCRect.TopLeft(),
                               {windowAreaCRect.right, windowAreaCRect.top},
                               windowAreaCRect.BottomRight(),
                               {windowAreaCRect.left, windowAreaCRect.bottom}};
  CPen WhitePen(
      PS_SOLID, 1,
  ...
  CRadarTarget rt;
  for (rt = radar_screen->GetPlugIn()->RadarTargetSelectFirst(); rt.IsValid();
       rt = radar_screen->GetPlugIn()->RadarTargetSelectNext(rt)) {
    int radarRange = radar_screen->CurrentConfig
                         ->getActiveProfile()["filters"]["radar_range_nm"]
                         .GetInt();

    if (rt.GetGS() < 60 || rt.GetPosition().GetPressureAltitude() > m_Filter ||
        !rt.IsValid() || !rt.GetPosition().IsValid() ||
        rt.GetPosition().GetPosition().DistanceTo(AptPositions[icao]) >
            radarRange)
      continue;
    ...
    for (int i = 1; i < radar_screen->Trail_App; i++) {
      ...
      if (Is_Inside(hPoint, appAreaVect)) {
        dc.SetPixel(hPoint, radar_screen->ColorManager
                                ->get_corrected_color("symbol", Color::White)
                                .ToCOLORREF());
      }
    ...
    if (Is_Inside(RtPoint, appAreaVect)) {
      ...
      CRect TargetArea(RtPoint.x - 4, RtPoint.y - 4, RtPoint.x + 4,
                       RtPoint.y + 4);
      TargetArea.NormalizeRect();
      radar_screen->AddScreenObject(
          DRAWING_AC_SYMBOL_APPWINDOW_BASE + (m_Id - APPWINDOW_BASE),
          rt.GetCallsign(), TargetArea, false,
          radar_screen->GetBottomLine(rt.GetCallsign()).c_str());
    }
    ...
    if (radar_screen->PredictedLength > 0) {
      ...
      d = double(rt.GetPosition().GetReportedGS() * 0.514444) *
              (radar_screen->PredictedLength * 60) -
          10;
      ...
      if (LiangBarsky(m_Area, projectPoint(AwayBase),
                      projectPoint(PredictedEnd), liangOne, liangTwo)) {
        dc.SelectObject(&WhitePen);
        dc.MoveTo(liangOne);
        dc.LineTo(liangTwo);
      }
    ...
    if (mouseWithin(mouseLocation, {RtPoint.x - 4, RtPoint.y - 4, RtPoint.x + 4,
                                    RtPoint.y + 4})) {
    ...
    TagCenter.x =
        long(RtPoint.x +
             float(lenght * cos(DegToRad(m_TagAngles[rt.GetCallsign()]))));
    TagCenter.y =
        long(RtPoint.y +
    ...
    map<string, string> TagReplacingMap = CSMRRadar::GenerateTagData(
        ...
        radar_screen->CurrentConfig
            ->getActiveProfile()["labels"]["use_aspeed_for_gate"]
            .GetBool(),
        icao, radar_screen->showAircraftType, radar_screen->showSID,
        radar_screen->showWakeTurb);
    ...
    TagClickableMap[TagReplacingMap["sid"]] =
        TagClickableMap[TagReplacingMap["shid"]] = TAG_CITEM_SID;
    ...
    RectF mesureRect;
    gdi->MeasureString(L" ", wcslen(L" "),
                       radar_screen->customFonts[radar_screen->currentFontSize],
                       PointF(0, 0), &Gdiplus::StringFormat(), &mesureRect);
    int blankWidth = (int)mesureRect.GetRight();

    mesureRect = RectF(0, 0, 0, 0);
    gdi->MeasureString(L"AZERTYUIOPQSDFGHJKLMWXCVBN",
                       wcslen(L"AZERTYUIOPQSDFGHJKLMWXCVBN"),
                       radar_screen->customFonts[radar_screen->currentFontSize],
                       PointF(0, 0), &Gdiplus::StringFormat(), &mesureRect);
    int oneLineHeight = (int)mesureRect.GetBottom();

    const Value &LabelsSettings =
        radar_screen->CurrentConfig->getActiveProfile()["labels"];
    ...
    bool rimcasLabelOnly =
        radar_screen->CurrentConfig
            ->getActiveProfile()["rimcas"]["rimcas_label_only"]
            .GetBool();

    Color definedBackgroundColor = radar_screen->CurrentConfig->getConfigColor(
        LabelsSettings[Utils::getEnumString(ColorTagType).c_str()]
                      ["background_color"]);
    if (TagType == CSMRRadar::TagTypes::Departure) {
      if (!TagReplacingMap["sid"].empty() &&
          radar_screen->CurrentConfig->isSidColorAvail(
              TagReplacingMap["sid"], radar_screen->getActiveAirport())) {
        definedBackgroundColor = radar_screen->CurrentConfig->getSidColor(
            TagReplacingMap["sid"], radar_screen->getActiveAirport());
      }
      ...
      if (fp.GetFlightPlanData().GetPlanType()[0] == 'I' &&
          ...
          LabelsSettings[Utils::getEnumString(ColorTagType).c_str()].HasMember(
              "nosid_color")) {
        definedBackgroundColor = radar_screen->CurrentConfig->getConfigColor(
            LabelsSettings[Utils::getEnumString(ColorTagType).c_str()]
                          ["nosid_color"]);
      }
    }
    ...
    Color TagBackgroundColor = radar_screen->RimcasInstance->GetAircraftColor(
    ...
    if (Is_Inside(TagBackgroundRect.TopLeft(), appAreaVect) &&
      ...
      SolidBrush FontColor(radar_screen->ColorManager->get_corrected_color(
          "label",
          radar_screen->CurrentConfig->getConfigColor(
              LabelsSettings[Utils::getEnumString(ColorTagType).c_str()]
                            ["text_color"])));
      SolidBrush SquawkErrorColor(
          radar_screen->ColorManager->get_corrected_color(
              "label", radar_screen->CurrentConfig->getConfigColor(
                           LabelsSettings["squawk_error_color"])));
      SolidBrush RimcasTextColor(radar_screen->CurrentConfig->getConfigColor(
          radar_screen->CurrentConfig
              ->getActiveProfile()["rimcas"]["alert_text_color"]));
      ...
      for (auto &&line : ReplacedLabelLines) {
        ...
        for (auto &&element : line) {
          ...
          if (radar_screen->RimcasInstance->getAlert(rt.GetCallsign()) !=
              CRimcas::NoAlert)
          ...
          radar_screen->AddScreenObject(
              TagClickableMap[element], rt.GetCallsign(), ItemRect, false,
              radar_screen->GetBottomLine(rt.GetCallsign()).c_str());
      ...
      POINT toDraw1, toDraw2;
      if (LiangBarsky(TagBackRectData, RtPoint, TagBackgroundRect.CenterPoint(),
                      toDraw1, toDraw2))
        gdi->DrawLine(
            &Pen(radar_screen->ColorManager->get_corrected_color("symbol",
                                                                 Color::White)),
            PointF(Gdiplus::REAL(RtPoint.x), Gdiplus::REAL(RtPoint.y)),
      ...
      if (rimcasLabelOnly) {
        ...
        if (RimcasLabelColor.ToCOLORREF() !=
          ...
          gdi->MeasureString(
              wrimcas_height.c_str(), wcslen(wrimcas_height.c_str()),
              radar_screen->customFonts[radar_screen->currentFontSize],
          ...
          gdi->FillRectangle(&SolidBrush(RimcasLabelColor),
      ...
      // radar_screen->AddScreenObject(DRAWING_TAG, rt.GetCallsign(),
      // TagBackgroundRect, true, GetBottomLine(rt.GetCallsign()).c_str());
  ...
  for (auto &&kv : DistanceTools) {
    CRadarTarget one =
        radar_screen->GetPlugIn()->RadarTargetSelect(kv.first.c_str());
    CRadarTarget two =
        radar_screen->GetPlugIn()->RadarTargetSelect(kv.second.c_str());
    ...
    if (one.GetGS() < 60 ||
        one.GetPosition().GetPressureAltitude() > m_Filter || !one.IsValid() ||
        !one.GetPosition().IsValid() ||
        one.GetPosition().GetPosition().DistanceTo(AptPositions[icao]) >
            radarRange)
      continue;

    if (two.GetGS() < 60 ||
        two.GetPosition().GetPressureAltitude() > m_Filter || !two.IsValid() ||
        !two.GetPosition().IsValid() ||
        two.GetPosition().GetPosition().DistanceTo(AptPositions[icao]) >
            radarRange)
      continue;
    ...
    if (Is_Inside(ClickableRect.TopLeft(), appAreaVect) &&
        Is_Inside(ClickableRect.BottomRight(), appAreaVect)) {
      gdi->FillRectangle(&SolidBrush(Color(127, 122, 122)),
                         CopyRect(ClickableRect));
      dc.Draw3dRect(ClickableRect, RGB(75, 75, 75), RGB(45, 45, 45));
      dc.TextOutA(TextPos.x, TextPos.y, text.c_str());

      radar_screen->AddScreenObject(RIMCAS_DISTANCE_TOOL,
                                    string(kv.first + "," + kv.second).c_str(),
                                    ClickableRect, false, "");
    }
  ...
  POINT TopLeftText = {TopBar.left + 5,
                       TopBar.bottom - dc.GetTextExtent("SRW 1").cy};
  ...
  dc.TextOutA(TopLeftText.x + (TopBar.right - TopBar.left) / 2 -
                  dc.GetTextExtent("SRW 1").cx,
              TopLeftText.y, Toptext.c_str());

  // Range button
  CRect RangeRect =
      Utils::drawToolbarButton(&dc, "Z", TopBar, 29, mouseLocation);
  radar_screen->AddScreenObject(m_Id, "range", RangeRect, false, "");

  // Filter button
  CRect FilterRect =
      Utils::drawToolbarButton(&dc, "F", TopBar, 42, mouseLocation);
  radar_screen->AddScreenObject(m_Id, "filter", FilterRect, false, "");

  // Rotate button
  CRect RotateRect =
      Utils::drawToolbarButton(&dc, "R", TopBar, 55, mouseLocation);
