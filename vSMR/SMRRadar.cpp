...
bool standardCursor; // switches between mouse cursor and pointer cursor when
                     // moving tags
...
inline bool IsTagBeingDragged(string c) { return TagBeingDragged == c; }
bool mouseWithin(CRect rect) {
  if (mouseLocation.x >= rect.left + 1 && mouseLocation.x <= rect.right - 1 &&
      mouseLocation.y >= rect.top + 1 && mouseLocation.y <= rect.bottom - 1)
    return true;
  return false;
}
...
CSMRRadar::CSMRRadar() {
  ...
  possible_paths[1] = fs::path(DllPath).parent_path().parent_path() /
                      fs::path("ICAO") / fs::path("ICAO_Airlines.txt");
  possible_paths[2] =
      fs::path(DllPath).parent_path().parent_path().parent_path() /
      fs::path("ICAO") / fs::path("ICAO_Airlines.txt");
  ...
  while (getline(ss, line, '\n')) {
    ...
    if (startsWith("COORD:", line.c_str())) {
      CPosition pos;
      pos.LoadFromStrings(line.substr(21, 14).c_str(),
                          line.substr(6, 14).c_str());
...
void CSMRRadar::CorrelateCursor() {
  if (NeedCorrelateCursor) {
    if (standardCursor) {
      smrCursor = CopyCursor((HCURSOR)::LoadImage(
          AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_SMRCORRELATE),
    ...
    if (!standardCursor) {
      if (customCursor) {
        smrCursor = CopyCursor((HCURSOR)::LoadImage(
            AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_SMRCURSOR),
            IMAGE_CURSOR, 0, 0, LR_SHARED));
      } else {
...
void CSMRRadar::LoadCustomFont() {
  ...
  string font_name =
      CurrentConfig->getActiveProfile()["font"]["font_name"].GetString();
  wstring buffer = wstring(font_name.begin(), font_name.end());
  Gdiplus::FontStyle fontStyle = Gdiplus::FontStyleRegular;
  if (strcmp(CurrentConfig->getActiveProfile()["font"]["weight"].GetString(),
             "Bold") == 0)
  ...
  customFonts[1] =
      new Gdiplus::Font(buffer.c_str(), Gdiplus::REAL(FSizes["one"].GetInt()),
                        fontStyle, Gdiplus::UnitPixel);
  customFonts[2] =
      new Gdiplus::Font(buffer.c_str(), Gdiplus::REAL(FSizes["two"].GetInt()),
                        fontStyle, Gdiplus::UnitPixel);
  customFonts[3] =
      new Gdiplus::Font(buffer.c_str(), Gdiplus::REAL(FSizes["three"].GetInt()),
                        fontStyle, Gdiplus::UnitPixel);
  customFonts[4] =
      new Gdiplus::Font(buffer.c_str(), Gdiplus::REAL(FSizes["four"].GetInt()),
                        fontStyle, Gdiplus::UnitPixel);
  customFonts[5] =
      new Gdiplus::Font(buffer.c_str(), Gdiplus::REAL(FSizes["five"].GetInt()),
                        fontStyle, Gdiplus::UnitPixel);
}
...
void CSMRRadar::LoadProfile(string profileName) {
  ...
  const Value &RimcasTimer =
      CurrentConfig->getActiveProfile()["rimcas"]["timer"];
  const Value &RimcasTimerLVP =
      CurrentConfig->getActiveProfile()["rimcas"]["timer_lvp"];
  ...
  LeaderLineDefaultlenght =
      CurrentConfig->getActiveProfile()["labels"]["leader_line_length"]
          .GetInt();
...
void CSMRRadar::OnAsrContentLoaded(bool Loaded) {
  ...
  if ((p_value = GetDataFromAsr("DepWindowShow")) != NULL)
    showDepartureWindow = atoi(p_value) == 1 ? true : false;

  if ((p_value = GetDataFromAsr("DepWindowDuration")) != NULL)
    departureDisplayDuration = atoi(p_value);

  if ((p_value = GetDataFromAsr("DepWindowColCallsign")) != NULL)
    depWindowShowCallsign = atoi(p_value) == 1 ? true : false;

  if ((p_value = GetDataFromAsr("DepWindowColDest")) != NULL)
    depWindowShowDest = atoi(p_value) == 1 ? true : false;

  if ((p_value = GetDataFromAsr("DepWindowColAcType")) != NULL)
    depWindowShowAcType = atoi(p_value) == 1 ? true : false;

  if ((p_value = GetDataFromAsr("DepWindowColWake")) != NULL)
    depWindowShowWake = atoi(p_value) == 1 ? true : false;

  if ((p_value = GetDataFromAsr("DepWindowColFreq")) != NULL)
    depWindowShowFreq = atoi(p_value) == 1 ? true : false;

  if ((p_value = GetDataFromAsr("DepWindowColTime")) != NULL)
    depWindowShowTime = atoi(p_value) == 1 ? true : false;

  if ((p_value = GetDataFromAsr("DepWindowPosX")) != NULL)
    DepartureWindowArea.left = atoi(p_value);

  if ((p_value = GetDataFromAsr("DepWindowPosY")) != NULL)
    DepartureWindowArea.top = atoi(p_value);
  ...
  for (int i = 1; i < 3; i++) {
    ...
    if ((p_value = GetDataFromAsr(string(prefix + "BottomRightX").c_str())) !=
        NULL)
      appWindows[i]->m_Area.right = atoi(p_value);

    if ((p_value = GetDataFromAsr(string(prefix + "BottomRightY").c_str())) !=
        NULL)
  ...
  for (rwy = GetPlugIn()->SectorFileElementSelectFirst(SECTOR_ELEMENT_RUNWAY);
    ...
    if (startsWith(getActiveAirport().c_str(), rwy.GetAirportName())) {
      ...
      if (rwy.IsElementActive(true, 0) || rwy.IsElementActive(true, 1) ||
          rwy.IsElementActive(false, 0) || rwy.IsElementActive(false, 1)) {
...
void CSMRRadar::OnAsrContentToBeSaved() {
  ...
  SaveDataToAsr("ActiveProfile", "vSMR active profile",
                CurrentConfig->getActiveProfileName().c_str());

  SaveDataToAsr("FontSize", "vSMR font size",
                std::to_string(currentFontSize).c_str());

  SaveDataToAsr("Afterglow", "vSMR Afterglow enabled",
                std::to_string(int(Afterglow)).c_str());

  SaveDataToAsr("AppTrailsDots", "vSMR APPR Trail Dots",
                std::to_string(Trail_App).c_str());

  SaveDataToAsr("GndTrailsDots", "vSMR GRND Trail Dots",
                std::to_string(Trail_Gnd).c_str());

  SaveDataToAsr("PredictedLine", "vSMR Predicted Track Lines",
                std::to_string(PredictedLength).c_str());

  SaveDataToAsr("WIPareas", "vSMR WIP Areas",
                std::to_string(wipAreasActive).c_str());

  SaveDataToAsr("ShowAircraftType", "Show Aircraft Type",
                std::to_string(showAircraftType).c_str());

  SaveDataToAsr("ShowSID", "Show SID", std::to_string(showSID).c_str());

  SaveDataToAsr("ShowWakeTurb", "Show Wake Turbulence",
                std::to_string(showWakeTurb).c_str());

  // Departure Timer Window settings
  SaveDataToAsr("DepWindowShow", "Show Departure Timer Window",
                std::to_string(int(showDepartureWindow)).c_str());
  SaveDataToAsr("DepWindowDuration", "Departure Timer Display Duration",
                std::to_string(departureDisplayDuration).c_str());
  SaveDataToAsr("DepWindowColCallsign", "Departure Timer Show Callsign",
                std::to_string(int(depWindowShowCallsign)).c_str());
  SaveDataToAsr("DepWindowColDest", "Departure Timer Show Destination",
                std::to_string(int(depWindowShowDest)).c_str());
  SaveDataToAsr("DepWindowColAcType", "Departure Timer Show AC Type",
                std::to_string(int(depWindowShowAcType)).c_str());
  SaveDataToAsr("DepWindowColWake", "Departure Timer Show Wake Cat",
                std::to_string(int(depWindowShowWake)).c_str());
  SaveDataToAsr("DepWindowColFreq", "Departure Timer Show Freq",
                std::to_string(int(depWindowShowFreq)).c_str());
  SaveDataToAsr("DepWindowColTime", "Departure Timer Show Time",
                std::to_string(int(depWindowShowTime)).c_str());
  SaveDataToAsr("DepWindowPosX", "Departure Timer Window X",
                std::to_string(DepartureWindowArea.left).c_str());
  SaveDataToAsr("DepWindowPosY", "Departure Timer Window Y",
                std::to_string(DepartureWindowArea.top).c_str());
  ...
  for (int i = 1; i < 3; i++) {
    ...
    SaveDataToAsr(string(prefix + "TopLeftX").c_str(), "SRW position",
                  temp.c_str());

    temp = std::to_string(appWindows[i]->m_Area.top);
    SaveDataToAsr(string(prefix + "TopLeftY").c_str(), "SRW position",
                  temp.c_str());

    temp = std::to_string(appWindows[i]->m_Area.right);
    SaveDataToAsr(string(prefix + "BottomRightX").c_str(), "SRW position",
                  temp.c_str());

    temp = std::to_string(appWindows[i]->m_Area.bottom);
    SaveDataToAsr(string(prefix + "BottomRightY").c_str(), "SRW position",
                  temp.c_str());

    temp = std::to_string(appWindows[i]->m_Offset.x);
    SaveDataToAsr(string(prefix + "OffsetX").c_str(), "SRW offset",
                  temp.c_str());

    temp = std::to_string(appWindows[i]->m_Offset.y);
    SaveDataToAsr(string(prefix + "OffsetY").c_str(), "SRW offset",
                  temp.c_str());

    temp = std::to_string(appWindows[i]->m_Filter);
    SaveDataToAsr(string(prefix + "Filter").c_str(), "SRW filter",
                  temp.c_str());
    ...
    SaveDataToAsr(string(prefix + "Rotation").c_str(), "SRW rotation",
                  temp.c_str());

    string to_save = "0";
    if (appWindowDisplays[i])
      to_save = "1";
    SaveDataToAsr(string(prefix + "Display").c_str(),
                  "Display Secondary Radar Window", to_save.c_str());
  }
}
...
void CSMRRadar::OnMoveScreenObject(int ObjectType, const char *sObjectId,
  ...
  if (ObjectType == APPWINDOW_ONE || ObjectType == APPWINDOW_TWO) {
    ...
    bool toggleCursor = appWindows[appWindowId]->OnMoveScreenObject(
        sObjectId, Pt, Area, Released);
    ...
    if (!toggleCursor) {
      if (standardCursor) {
        if (strcmp(sObjectId, "topbar") == 0)
          smrCursor = CopyCursor((HCURSOR)::LoadImage(
              AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_SMRMOVEWINDOW),
              IMAGE_CURSOR, 0, 0, LR_SHARED));
        else if (strcmp(sObjectId, "resize") == 0)
          smrCursor = CopyCursor((HCURSOR)::LoadImage(
              AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_SMRRESIZE),
              IMAGE_CURSOR, 0, 0, LR_SHARED));
  ...
  if (ObjectType == DRAWING_TAG || ObjectType == TAG_CITEM_MANUALCORRELATE ||
      ObjectType == TAG_CITEM_CALLSIGN || ObjectType == TAG_CITEM_FPBOX ||
      ObjectType == TAG_CITEM_RWY || ObjectType == TAG_CITEM_SID ||
      ObjectType == TAG_CITEM_GATE || ObjectType == TAG_CITEM_NO ||
      ObjectType == TAG_CITEM_GROUNDSTATUS) {
    CRadarTarget rt = GetPlugIn()->RadarTargetSelect(sObjectId);

    if (!Released) {
      if (standardCursor) {
        smrCursor = CopyCursor((HCURSOR)::LoadImage(
            AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_SMRMOVETAG),
            IMAGE_CURSOR, 0, 0, LR_SHARED));
        AFX_MANAGE_STATE(AfxGetStaticModuleState());
        ASSERT(smrCursor);
        SetCursor(smrCursor);
    ...
    if (rt.IsValid()) {
      ...
      POINT AcPosPix =
          ConvertCoordFromPositionToPixel(GetPlugIn()
                                              ->RadarTargetSelect(sObjectId)
                                              .GetPosition()
                                              .GetPosition());
      POINT CustomTag = {TagCenterPix.x - AcPosPix.x,
                         TagCenterPix.y - AcPosPix.y};
      ...
      if (CurrentConfig->getActiveProfile()["labels"]["auto_deconfliction"]
        ...
        TagLeaderLineLength[sObjectId] =
            max(LeaderLineDefaultlenght,
  ...
  // Handle Departure Timer Window movement
  if (ObjectType == RIMCAS_DEP_WINDOW) {
    DepartureWindowArea = Area;
    OnAsrContentToBeSaved();
...
void CSMRRadar::OnClickScreenObject(int ObjectType, const char *sObjectId,
  ...
  if (ObjectType == APPWINDOW_ONE || ObjectType == APPWINDOW_TWO) {
    ...
    if (strcmp(sObjectId, "range") == 0) {
      ...
      GetPlugIn()->AddPopupListElement(
          "55", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 55));
      GetPlugIn()->AddPopupListElement(
          "50", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 50));
      GetPlugIn()->AddPopupListElement(
          "45", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 45));
      GetPlugIn()->AddPopupListElement(
          "40", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 40));
      GetPlugIn()->AddPopupListElement(
          "35", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 35));
      GetPlugIn()->AddPopupListElement(
          "30", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 30));
      GetPlugIn()->AddPopupListElement(
          "25", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 25));
      GetPlugIn()->AddPopupListElement(
          "20", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 20));
      GetPlugIn()->AddPopupListElement(
          "15", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 15));
      GetPlugIn()->AddPopupListElement(
          "10", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 10));
      GetPlugIn()->AddPopupListElement(
          "5", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 5));
      GetPlugIn()->AddPopupListElement(
          "1", "", RIMCAS_UPDATERANGE + appWindowId, false,
          int(appWindows[appWindowId]->m_Scale == 1));
      GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2,
                                       false, true);
    }
    if (strcmp(sObjectId, "filter") == 0) {
      GetPlugIn()->OpenPopupList(Area, "SRW Filter (ft)", 1);
      GetPlugIn()->AddPopupListElement(
          "UNL", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 66000));
      GetPlugIn()->AddPopupListElement(
          "9500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 9500));
      GetPlugIn()->AddPopupListElement(
          "8500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 8500));
      GetPlugIn()->AddPopupListElement(
          "7500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 7500));
      GetPlugIn()->AddPopupListElement(
          "6500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 6500));
      GetPlugIn()->AddPopupListElement(
          "5500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 5500));
      GetPlugIn()->AddPopupListElement(
          "4500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 4500));
      GetPlugIn()->AddPopupListElement(
          "3500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 3500));
      GetPlugIn()->AddPopupListElement(
          "2500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 2500));
      GetPlugIn()->AddPopupListElement(
          "1500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 1500));
      GetPlugIn()->AddPopupListElement(
          "500", "", RIMCAS_UPDATEFILTER + appWindowId, false,
          int(appWindows[appWindowId]->m_Filter == 500));
      string tmp = std::to_string(GetPlugIn()->GetTransitionAltitude());
      GetPlugIn()->AddPopupListElement(tmp.c_str(), "",
                                       RIMCAS_UPDATEFILTER + appWindowId, false,
                                       2, false, true);
      GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2,
                                       false, true);
    }
    if (strcmp(sObjectId, "rotate") == 0) {
      ...
      for (int k = 0; k <= 360; k++) {
        ...
        GetPlugIn()->AddPopupListElement(
            tmp.c_str(), "", RIMCAS_UPDATEROTATE + appWindowId, false,
  ...
  if (ObjectType == RIMCAS_ACTIVE_AIRPORT) {
    GetPlugIn()->OpenPopupEdit(Area, RIMCAS_ACTIVE_AIRPORT_FUNC,
                               getActiveAirport().c_str());
  }
  ...
  if (ObjectType == RIMCAS_MENU) {
    ...
    if (strcmp(sObjectId, "DisplayMenu") == 0) {
      ...
      GetPlugIn()->AddPopupListElement("QDR Fixed Reference", "",
                                       RIMCAS_QDM_TOGGLE);
      GetPlugIn()->AddPopupListElement("QDR Select Reference", "",
                                       RIMCAS_QDM_SELECT_TOGGLE);
      GetPlugIn()->AddPopupListElement("SRW 1", "", APPWINDOW_ONE, false,
                                       int(appWindowDisplays[1]));
      GetPlugIn()->AddPopupListElement("SRW 2", "", APPWINDOW_TWO, false,
                                       int(appWindowDisplays[2]));
      GetPlugIn()->AddPopupListElement("WIP Areas", "", WIP_AREAS, false,
                                       int(wipAreasActive));
      GetPlugIn()->AddPopupListElement("Departure Timer", "",
                                       RIMCAS_DEP_WINDOW_TOGGLE, false,
                                       int(showDepartureWindow));
      GetPlugIn()->AddPopupListElement("Show Aircraft Type", "",
                                       RIMCAS_TOGGLE_AIRCRAFT_TYPE, false,
                                       int(showAircraftType));
      GetPlugIn()->AddPopupListElement("Show SID", "", RIMCAS_TOGGLE_SID, false,
                                       int(showSID));
      GetPlugIn()->AddPopupListElement("Show Wake Catagory", "",
                                       RIMCAS_TOGGLE_WAKE_TURB, false,
                                       int(showWakeTurb));
    ...
    if (strcmp(sObjectId, "TargetMenu") == 0) {
      ...
      GetPlugIn()->AddPopupListElement("Afterglow", "", RIMCAS_UPDATE_AFTERGLOW,
                                       false, int(Afterglow));
      ...
      GetPlugIn()->AddPopupListElement("Predicted Track Line", "",
                                       RIMCAS_OPEN_LIST);
    ...
    if (strcmp(sObjectId, "RIMCASMenu") == 0) {
      ...
      GetPlugIn()->AddPopupListElement("Conflict Alert ARR", "",
                                       RIMCAS_OPEN_LIST);
      GetPlugIn()->AddPopupListElement("Conflict Alert DEP", "",
                                       RIMCAS_OPEN_LIST);
      ...
      GetPlugIn()->AddPopupListElement("Departure Timer", "", RIMCAS_OPEN_LIST);
      GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2,
                                       false, true);
    }
  ...
  if (ObjectType == DRAWING_TAG || ObjectType == DRAWING_AC_SYMBOL) {
    CRadarTarget rt = GetPlugIn()->RadarTargetSelect(sObjectId);
    // GetPlugIn()->SetASELAircraft(rt); // NOTE: This does NOT work eventhough
    // the api says it should?
    GetPlugIn()->SetASELAircraft(GetPlugIn()->FlightPlanSelect(
        sObjectId)); // make sure the correct aircraft is selected before
                     // calling 'StartTagFunction'

    if (rt.GetCorrelatedFlightPlan().IsValid()) {
      StartTagFunction(rt.GetCallsign(), NULL,
                       EuroScopePlugIn::TAG_ITEM_TYPE_CALLSIGN,
                       rt.GetCallsign(), NULL,
                       EuroScopePlugIn::TAG_ITEM_FUNCTION_NO, Pt, Area);
    }
    ...
    if (ReleaseInProgress || AcquireInProgress) {
      if (ReleaseInProgress) {
        ...
        if (std::find(ManuallyCorrelated.begin(), ManuallyCorrelated.end(),
          ...
          ManuallyCorrelated.erase(std::find(ManuallyCorrelated.begin(),
      ...
      if (AcquireInProgress) {
        ...
        if (std::find(ReleasedTracks.begin(), ReleasedTracks.end(),
          ...
          ReleasedTracks.erase(std::find(
              ReleasedTracks.begin(), ReleasedTracks.end(), rt.GetSystemID()));
      }
  ...
  if (Button == BUTTON_LEFT) {
    ...
    if (rt.GetCorrelatedFlightPlan().IsValid()) {
      StartTagFunction(rt.GetCallsign(), NULL, TAG_ITEM_TYPE_CALLSIGN,
                       rt.GetCallsign(), NULL, TAG_ITEM_FUNCTION_NO, Pt, Area);
    }
  }
  ...
  if (Button == BUTTON_MIDDLE && TagObjectMiddleTypes[ObjectType]) {
    ...
    GetPlugIn()->SetASELAircraft(GetPlugIn()->FlightPlanSelect(sObjectId));
    StartTagFunction(rt.GetCallsign(), NULL,
                     EuroScopePlugIn::TAG_ITEM_TYPE_CALLSIGN, rt.GetCallsign(),
                     NULL, TagMenu, Pt, Area);
  }
  ...
  if (ObjectType == RIMCAS_DISTANCE_TOOL) {
    ...
    std::pair<iterator, iterator> iterpair =
        DistanceTools.equal_range(toRemove.first);
...
void CSMRRadar::OnFunctionCall(int FunctionId, const char *sItemString,
  ...
  if (FunctionId == RIMCAS_QDM_SELECT_TOGGLE) {
    if (!QDMSelectEnabled) {
      QDMSelectPt =
          ConvertCoordFromPositionToPixel(AirportPositions[getActiveAirport()]);
    }
  ...
  if (FunctionId == RIMCAS_UPDATEFILTER1 ||
      FunctionId == RIMCAS_UPDATEFILTER2) {
  ...
  if (FunctionId == RIMCAS_UPDATEROTATE1 ||
  ...
  // Departure Timer Window function handlers
  if (FunctionId == RIMCAS_DEP_WINDOW_TOGGLE) {
    showDepartureWindow = !showDepartureWindow;
    OnAsrContentToBeSaved();
  }

  if (FunctionId == RIMCAS_DEP_WINDOW_DURATION) {
    departureDisplayDuration = atoi(sItemString);
    OnAsrContentToBeSaved();
    ShowLists["Departure Timer"] = true;
  }

  if (FunctionId == RIMCAS_DEP_WINDOW_COL_CALLSIGN) {
    depWindowShowCallsign = !depWindowShowCallsign;
    OnAsrContentToBeSaved();
    ShowLists["Departure Timer"] = true;
  }

  if (FunctionId == RIMCAS_DEP_WINDOW_COL_DEST) {
    depWindowShowDest = !depWindowShowDest;
    OnAsrContentToBeSaved();
    ShowLists["Departure Timer"] = true;
  }

  if (FunctionId == RIMCAS_DEP_WINDOW_COL_ACTYPE) {
    depWindowShowAcType = !depWindowShowAcType;
    OnAsrContentToBeSaved();
    ShowLists["Departure Timer"] = true;
  }

  if (FunctionId == RIMCAS_DEP_WINDOW_COL_WAKE) {
    depWindowShowWake = !depWindowShowWake;
    OnAsrContentToBeSaved();
    ShowLists["Departure Timer"] = true;
  }

  if (FunctionId == RIMCAS_DEP_WINDOW_COL_FREQ) {
    depWindowShowFreq = !depWindowShowFreq;
    OnAsrContentToBeSaved();
    ShowLists["Departure Timer"] = true;
  }

  if (FunctionId == RIMCAS_DEP_WINDOW_COL_TIME) {
    depWindowShowTime = !depWindowShowTime;
    OnAsrContentToBeSaved();
    ShowLists["Departure Timer"] = true;
  }
...
void CSMRRadar::RefreshAirportActivity(void) {
  ...
  for (airport =
           GetPlugIn()->SectorFileElementSelectFirst(SECTOR_ELEMENT_AIRPORT);
       airport.IsValid(); airport = GetPlugIn()->SectorFileElementSelectNext(
                              airport, SECTOR_ELEMENT_AIRPORT)) {
...
void CSMRRadar::OnRadarTargetPositionUpdate(CRadarTarget RadarTarget) {
  ...
  Patatoides[RadarTarget.GetCallsign()].History_three_points =
      Patatoides[RadarTarget.GetCallsign()].History_two_points;
  Patatoides[RadarTarget.GetCallsign()].History_two_points =
      Patatoides[RadarTarget.GetCallsign()].History_one_points;
  Patatoides[RadarTarget.GetCallsign()].History_one_points =
      Patatoides[RadarTarget.GetCallsign()].points;

  Patatoides[RadarTarget.GetCallsign()].points.clear();

  CFlightPlan fp = GetPlugIn()->FlightPlanSelect(RadarTarget.GetCallsign());

  if ((getActiveAirport() == "EGLL" || getActiveAirport() == "EGKK") &&
      CurrentConfig->isPositionInGeofenceArea(
          getActiveAirport(), RtPos.GetPosition())) { // If on stand at Heathrow
    int c = 0;
    CPosition centre = RtPos.GetPosition();

    for (int hdg = 0; hdg <= 360; hdg += 90) {
      CPosition pos = Haversine(centre, hdg, 14.0f);
      Patatoides[RadarTarget.GetCallsign()].points[c++] = {pos.m_Latitude,
                                                           pos.m_Longitude};
    }
  ...
  if (fp.IsValid()) {
    ...
    CAircraftTypeLookup::AircraftData data =
        AircraftTypes->getAircraftData(shortACType);
  ...
  float trackHead =
      float(RadarTarget.GetPosition().GetReportedHeadingTrueNorth());
  ...
  CPosition bottomMiddle =
      Haversine(RtPos.GetPosition(), inverseTrackHead, HalfLenght);
  ...
  CPosition middleTopLeft = Haversine(
      topLeft, float(fmod(inverseTrackHead + 25.0f, 360)), 0.8f * HalfLenght);
  CPosition middleTopRight = Haversine(
      topRight, float(fmod(inverseTrackHead - 25.0f, 360)), 0.8f * HalfLenght);
  CPosition middleBottomLeft = Haversine(
      bottomLeft, float(fmod(trackHead - 15.0f, 360)), 0.8f * HalfLenght);
  CPosition middleBottomRight = Haversine(
      bottomRight, float(fmod(trackHead + 15.0f, 360)), 0.8f * HalfLenght);

  CPosition rightTop =
      Haversine(middleBottomRight, rightTrackHead, 0.7f * HalfSpanWidth);
  CPosition rightBottom = Haversine(rightTop, inverseTrackHead, cabin_width);

  CPosition leftTop =
      Haversine(middleBottomLeft, leftTrackHead, 0.7f * HalfSpanWidth);
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (i == 11)
      endPoint = basePoints[0];
    else
      endPoint = basePoints[i + 1];

    double dist, rndHeading;
    dist = startPoint.DistanceTo(endPoint);

    Patatoides[RadarTarget.GetCallsign()].points[i * 7] = {
        startPoint.m_Latitude, startPoint.m_Longitude};
    lastPoint = startPoint;

    for (int k = 1; k < 7; k++) {

      rndHeading = float(fmod(
          lastPoint.DirectionTo(endPoint) + (-25.0 + (rand() % 50 + 1)), 360));
      newPoint = Haversine(lastPoint, rndHeading, dist * 200);
      Patatoides[RadarTarget.GetCallsign()].points[(i * 7) + k] = {
          newPoint.m_Latitude, newPoint.m_Longitude};
      lastPoint = newPoint;
    }
  }
}
...
string CSMRRadar::GetBottomLine(const char *Callsign) {
  ...
  if (fp.IsValid()) {
    ...
    if (fp.GetFlightPlanData().IsReceived()) {
      ...
      const char *ssr = GetPlugIn()
                            ->RadarTargetSelect(fp.GetCallsign())
                            .GetPosition()
                            .GetSquawk();
...
map<string, string> CSMRRadar::GenerateTagData(
  ...
  // * flightlevel: Flightlevel/Pressure altitude of the ac * gs: Ground speed
  // of the ac * tendency: Climbing or descending symbol * wake: Wake turbulance
  // cat * groundstatus: Current status * ssr: the current squawk of the ac sid:
  // the assigned SID ssid: a short version of the SID origin: origin aerodrome
  ...
  // ----- Tendency -------
  string tendency = "-";
  int delta_fl = rt.GetPosition().GetFlightLevel() -
                 rt.GetPreviousPosition(rt.GetPosition()).GetFlightLevel();
  if (abs(delta_fl) >= 50) {
    if (delta_fl < 0) {
      tendency = "|";
    } else {
...
void CSMRRadar::OnFlightPlanDisconnect(CFlightPlan FlightPlan) {
  ...
  for (multimap<string, string>::iterator itr = DistanceTools.begin();
       itr != DistanceTools.end(); ++itr) {
    if (itr->first == callsign || itr->second == callsign)
      itr = DistanceTools.erase(itr);
  }

  // Remove from departure timer when aircraft disconnects
  RimcasInstance->DismissDeparture(callsign);

  // Clear the ground tracking state for this aircraft
  if (RimcasInstance->AircraftWasOnGround.find(callsign) !=
      RimcasInstance->AircraftWasOnGround.end()) {
    RimcasInstance->AircraftWasOnGround.erase(callsign);
  }
}
...
void CSMRRadar::OnRefresh(HDC hDC, int Phase) {
  ...
  if (initCursor) {
    if (customCursor) {
      smrCursor = CopyCursor((HCURSOR)::LoadImage(
          AfxGetInstanceHandle(), MAKEINTRESOURCE(IDC_SMRCURSOR), IMAGE_CURSOR,
          0, 0, LR_SHARED));
      // This got broken because of threading as far as I can tell
      // The cursor does change for some milliseconds but gets reset almost
      // instantly by external MFC code
    ...
    if (smrCursor != nullptr) {
      ...
      gSourceProc =
          (WNDPROC)SetWindowLong(pluginWindow, GWL_WNDPROC, (LONG)WindowProc);
    }
    initCursor = false;
  }
  ...
  if (Phase == REFRESH_PHASE_AFTER_LISTS) {
    ...
    if (!ColorSettingsDay) {
      ...
      SolidBrush AlphaBrush(Color(
          CurrentConfig->getActiveProfile()["filters"]["night_alpha_setting"]
              .GetInt(),
          0, 0, 0));
  ...
  struct Utils {
    static RECT GetAreaFromText(CDC *dc, string text, POINT Pos) {
      RECT Area = {Pos.x, Pos.y, Pos.x + dc->GetTextExtent(text.c_str()).cx,
                   Pos.y + dc->GetTextExtent(text.c_str()).cy};
      return Area;
    }
  ...
  for (apt = GetPlugIn()->SectorFileElementSelectFirst(SECTOR_ELEMENT_AIRPORT);
       apt.IsValid(); apt = GetPlugIn()->SectorFileElementSelectNext(
                          apt, SECTOR_ELEMENT_AIRPORT)) {
    CPosition Pos;
  ...
  for (rwy = GetPlugIn()->SectorFileElementSelectFirst(SECTOR_ELEMENT_RUNWAY);
    ...
    if (startsWith(getActiveAirport().c_str(), rwy.GetAirportName())) {
      ...
      const Value &CustomMap =
          CurrentConfig->getAirportMapIfAny(getActiveAirport());

      vector<CPosition> def;
      // Rimcas now ignores the defined runway polygon to ensure that the
      // correct detection area is used, defined runway is now only used for
      // closed runway
      //			if
      ...
      if (RimcasInstance->ClosedRunway.find(RwName) !=
          RimcasInstance->ClosedRunway.end()) {
        if (RimcasInstance->ClosedRunway[RwName]) {

          CPen RedPen(PS_SOLID, 2, RGB(150, 0, 0));
          CPen *oldPen = dc.SelectObject(&RedPen);

          if (CurrentConfig->isCustomRunwayAvail(getActiveAirport(),
                ...
                if (startsWith(runway_name.c_str(),
                  ...
                  for (SizeType w = 0; w < Path.Size(); w++) {
                    ...
                    position.LoadFromStrings(
                        Path[w][static_cast<SizeType>(1)].GetString(),
                  ...
                  graphics.FillPolygon(&SolidBrush(Color(150, 0, 0)), lpPoints,
                                       k - 1);
  ...
  for (rt = GetPlugIn()->RadarTargetSelectFirst(); rt.IsValid();
    ...
    if (rt.GetGS() > 5) {
      ...
      for (int i = 1; i <= 2; i++) {
        ...
        if (i == 1 &&
            !Patatoides[rt.GetCallsign()].History_one_points.empty() &&
            Afterglow &&
            CurrentConfig->getActiveProfile()["targets"]["show_primary_target"]
                .GetBool()) {
          SolidBrush H_Brush(ColorManager->get_corrected_color(
              "afterglow",
              CurrentConfig->getConfigColor(
                  CurrentConfig
                      ->getActiveProfile()["targets"]["history_one_color"])));

          PointF lpPoints[100];
          for (unsigned int i1 = 0;
               i1 < Patatoides[rt.GetCallsign()].History_one_points.size();
               i1++) {
            CPosition pos;
            pos.m_Latitude =
                Patatoides[rt.GetCallsign()].History_one_points[i1].x;
            pos.m_Longitude =
                Patatoides[rt.GetCallsign()].History_one_points[i1].y;

            lpPoints[i1] = {REAL(ConvertCoordFromPositionToPixel(pos).x),
                            REAL(ConvertCoordFromPositionToPixel(pos).y)};
          }
          graphics.FillPolygon(
              &H_Brush, lpPoints,
              Patatoides[rt.GetCallsign()].History_one_points.size());
        }
        ...
        if (i != 2) {
          if (!Patatoides[rt.GetCallsign()].History_two_points.empty() &&
              Afterglow &&
              CurrentConfig
                  ->getActiveProfile()["targets"]["show_primary_target"]
                  .GetBool()) {
            SolidBrush H_Brush(ColorManager->get_corrected_color(
                "afterglow",
                CurrentConfig->getConfigColor(
                    CurrentConfig
                        ->getActiveProfile()["targets"]["history_two_color"])));

            PointF lpPoints[100];
            for (unsigned int i1 = 0;
                 i1 < Patatoides[rt.GetCallsign()].History_two_points.size();
                 i1++) {
              CPosition pos;
              pos.m_Latitude =
                  Patatoides[rt.GetCallsign()].History_two_points[i1].x;
              pos.m_Longitude =
                  Patatoides[rt.GetCallsign()].History_two_points[i1].y;
        ...
        if (i == 2 &&
            !Patatoides[rt.GetCallsign()].History_three_points.empty() &&
            Afterglow &&
            CurrentConfig->getActiveProfile()["targets"]["show_primary_target"]
                .GetBool()) {
          SolidBrush H_Brush(ColorManager->get_corrected_color(
              ...
              CurrentConfig->getConfigColor(
                  CurrentConfig
                      ->getActiveProfile()["targets"]["history_three_color"])));

          PointF lpPoints[100];
          for (unsigned int i1 = 0;
               i1 < Patatoides[rt.GetCallsign()].History_three_points.size();
               i1++) {
            CPosition pos;
            pos.m_Latitude =
                Patatoides[rt.GetCallsign()].History_three_points[i1].x;
            pos.m_Longitude =
                Patatoides[rt.GetCallsign()].History_three_points[i1].y;
          ...
          graphics.FillPolygon(
              &H_Brush, lpPoints,
              Patatoides[rt.GetCallsign()].History_three_points.size());
        }
      }
      ...
      CRadarTargetPositionData previousPos =
          rt.GetPreviousPosition(rt.GetPosition());
      for (int j = 1; j <= TrailNumber; j++) {
        POINT pCoord =
            ConvertCoordFromPositionToPixel(previousPos.GetPosition());

        graphics.FillRectangle(&SolidBrush(ColorManager->get_corrected_color(
                                   "symbol", Gdiplus::Color::White)),
    ...
    if (CurrentConfig->getActiveProfile()["targets"]["show_primary_target"]
            .GetBool()) {
      PointF lpPoints[100];
      for (unsigned int i = 0; i < Patatoides[rt.GetCallsign()].points.size();
           i++) {
        CPosition pos;
        pos.m_Latitude = Patatoides[rt.GetCallsign()].points[i].x;
        pos.m_Longitude = Patatoides[rt.GetCallsign()].points[i].y;

        lpPoints[i] = {REAL(ConvertCoordFromPositionToPixel(pos).x),
                       REAL(ConvertCoordFromPositionToPixel(pos).y)};
      }

      if ((getActiveAirport() == "EGLL" || getActiveAirport() == "EGKK") &&
          CurrentConfig->isPositionInGeofenceArea(getActiveAirport(),
                                                  RtPos.GetPosition())) {
        SolidBrush H_Brush(ColorManager->get_corrected_color(
            "afterglow", Gdiplus::Color::White));
        graphics.FillPolygon(&H_Brush, lpPoints,
                             Patatoides[rt.GetCallsign()].points.size());

      } else {
        SolidBrush H_Brush(ColorManager->get_corrected_color(
            "afterglow",
            CurrentConfig->getConfigColor(
                CurrentConfig->getActiveProfile()["targets"]["target_color"])));
    ...
    bool AcisCorrelated =
        IsCorrelated(GetPlugIn()->FlightPlanSelect(rt.GetCallsign()), rt);

    if (!AcisCorrelated && reportedGs < 1 && !ReleaseInProgress &&
        !AcquireInProgress)
      continue;

    CPen qTrailPen(
        PS_SOLID, 1,
        ColorManager->get_corrected_color("symbol", Gdiplus::Color::White)
            .ToCOLORREF());
    ...
    if (reportedGs > 50 && PredictedLength > 0) {
      ...
      CPosition AwayBase = BetterHarversine(rt.GetPosition().GetPosition(),
                                            rt.GetTrackHeading(), d);

      d = double(rt.GetPosition().GetReportedGS() * 0.514444) *
              (PredictedLength * 60) -
          10;
      CPosition PredictedEnd =
          BetterHarversine(AwayBase, rt.GetTrackHeading(), d);
    ...
    AddScreenObject(
        DRAWING_AC_SYMBOL, rt.GetCallsign(),
        {acPosPix.x - 5, acPosPix.y - 5, acPosPix.x + 5, acPosPix.y + 5}, false,
        AcisCorrelated ? GetBottomLine(rt.GetCallsign()).c_str()
                       : rt.GetSystemID());
...
#pragma endregion Drawing of the symbols
  ...
  RimcasInstance->OnRefreshEnd(
      this,
      CurrentConfig
          ->getActiveProfile()["rimcas"]["rimcas_stage_two_speed_threshold"]
          .GetInt());
...
#pragma region tags
  ...
  for (rt = GetPlugIn()->RadarTargetSelectFirst(); rt.IsValid();
    ...
    if (CurrentConfig->isPositionInGeofenceArea(getActiveAirport(),
                                                RtPos.GetPosition()))
      isAcDisplayed = false;
    ...
    if (it != TagsOffsets.end()) {
      ...
      if (TagLeaderLineLength.find(rt.GetCallsign()) !=
          TagLeaderLineLength.end())
        lenght = TagLeaderLineLength[rt.GetCallsign()];

      TagCenter.x =
          long(acPosPix.x +
               float(lenght * cos(DegToRad(TagAngles[rt.GetCallsign()]))));
      TagCenter.y =
          long(acPosPix.y +
               float(lenght * sin(DegToRad(TagAngles[rt.GetCallsign()]))));
    }

    TagTypes TagType = TagTypes::Departure;
    TagTypes ColorTagType = TagTypes::Departure;

    if (fp.IsValid() && strcmp(fp.GetFlightPlanData().GetDestination(),
                               getActiveAirport().c_str()) == 0) {
      // Circuit aircraft are treated as departures; not arrivals
      if (strcmp(fp.GetFlightPlanData().GetOrigin(),
                 getActiveAirport().c_str()) != 0) {
    ...
    if (reportedGs > 50) {
      ...
      // Is "use_departure_arrival_coloring" enabled? if not, then use the
      // airborne colors
      bool useDepArrColors =
          CurrentConfig
              ->getActiveProfile()["labels"]["airborne"]
    ...
    map<string, string> TagReplacingMap = GenerateTagData(
        ...
        CurrentConfig->getActiveProfile()["labels"]["use_aspeed_for_gate"]
            .GetBool(),
        getActiveAirport(), showAircraftType, showSID, showWakeTurb);
    ...
    TagClickableMap[TagReplacingMap["asid"]] =
        TagClickableMap[TagReplacingMap["ssid"]] = TAG_CITEM_SID;
    ...
    RectF mesureRect;
    graphics.MeasureString(L" ", wcslen(L" "), customFonts[currentFontSize],
                           PointF(0, 0), &Gdiplus::StringFormat(), &mesureRect);
    int blankWidth = (int)mesureRect.GetRight();

    mesureRect = RectF(0, 0, 0, 0);
    graphics.MeasureString(L"AZERTYUIOPQSDFGHJKLMWXCVBN",
                           wcslen(L"AZERTYUIOPQSDFGHJKLMWXCVBN"),
                           customFonts[currentFontSize], PointF(0, 0),
                           &Gdiplus::StringFormat(), &mesureRect);
    int oneLineHeight = (int)mesureRect.GetBottom();

    const Value &LabelsSettings = CurrentConfig->getActiveProfile()["labels"];
    const Value &LabelLines =
        LabelsSettings[Utils::getEnumString(TagType).c_str()]["definition"];
    ...
    for (unsigned int i = 0; i < LabelLines.Size(); i++) {
      ...
      for (unsigned int j = 0; j < line.Size(); j++) {
        ...
        // Only add blank space if this element is not empty AND there's a next
        // element that's not empty
    ...
    Color definedBackgroundColor = CurrentConfig->getConfigColor(
        LabelsSettings[Utils::getEnumString(ColorTagType).c_str()]
                      ["background_color"]);

    if (ColorTagType == TagTypes::Departure) {
      if (!TagReplacingMap["asid"].empty() &&
          CurrentConfig->isSidColorAvail(TagReplacingMap["asid"],
                                         getActiveAirport())) {
        definedBackgroundColor = CurrentConfig->getSidColor(
            TagReplacingMap["asid"], getActiveAirport());
      }
      if (fp.GetFlightPlanData().GetPlanType()[0] == 'I' &&
          TagReplacingMap["asid"].empty() &&
          LabelsSettings[Utils::getEnumString(ColorTagType).c_str()].HasMember(
              "nosid_color")) {
    ...
    Color TagBackgroundColor = RimcasInstance->GetAircraftColor(
        rt.GetCallsign(), definedBackgroundColor,
        CurrentConfig->getConfigColor(
            LabelsSettings[Utils::getEnumString(ColorTagType).c_str()]
                          ["background_color_on_runway"]),
        CurrentConfig->getConfigColor(
            CurrentConfig
                ->getActiveProfile()["rimcas"]["background_color_stage_one"]),
        CurrentConfig->getConfigColor(
            CurrentConfig
                ->getActiveProfile()["rimcas"]["background_color_stage_two"]));

    // We need to figure out if the tag color changes according to RIMCAS
    // alerts, or not
    bool rimcasLabelOnly =
        CurrentConfig->getActiveProfile()["rimcas"]["rimcas_label_only"]
            .GetBool();

    if (rimcasLabelOnly)
      TagBackgroundColor = RimcasInstance->GetAircraftColor(
          rt.GetCallsign(), definedBackgroundColor,
          CurrentConfig->getConfigColor(
              LabelsSettings[Utils::getEnumString(ColorTagType).c_str()]
                            ["background_color_on_runway"]));
    ...
    TagBackgroundColor =
        ColorManager->get_corrected_color("label", TagBackgroundColor);

    // Drawing the tag background

    CRect TagBackgroundRect(
        TagCenter.x - (TagWidth / 2), TagCenter.y - (TagHeight / 2),
        TagCenter.x + (TagWidth / 2), TagCenter.y + (TagHeight / 2));
    ...
    SolidBrush FontColor(ColorManager->get_corrected_color(
        "label", CurrentConfig->getConfigColor(
                     LabelsSettings[Utils::getEnumString(ColorTagType).c_str()]
                                   ["text_color"])));
    SolidBrush SquawkErrorColor(ColorManager->get_corrected_color(
        "label",
        CurrentConfig->getConfigColor(LabelsSettings["squawk_error_color"])));
    SolidBrush RimcasTextColor(CurrentConfig->getConfigColor(
        CurrentConfig->getActiveProfile()["rimcas"]["alert_text_color"]));
    SolidBrush GroundPushColor(ColorManager->get_corrected_color(
        "label", CurrentConfig->getConfigColor(
                     LabelsSettings["groundstatus_colors"]["push"])));
    SolidBrush GroundTaxiColor(ColorManager->get_corrected_color(
        "label", CurrentConfig->getConfigColor(
                     LabelsSettings["groundstatus_colors"]["taxi"])));
    SolidBrush GroundDepaColor(ColorManager->get_corrected_color(
        "label", CurrentConfig->getConfigColor(
                     LabelsSettings["groundstatus_colors"]["depa"])));

    // Drawing the leader line
    RECT TagBackRectData = TagBackgroundRect;
    POINT toDraw1, toDraw2;
    if (LiangBarsky(TagBackRectData, acPosPix, TagBackgroundRect.CenterPoint(),
                    toDraw1, toDraw2))
      graphics.DrawLine(
          &Pen(ColorManager->get_corrected_color("symbol", Color::White)),
    ...
    if (rimcasLabelOnly) {
      Color RimcasLabelColor = RimcasInstance->GetAircraftColor(
          rt.GetCallsign(), Color::AliceBlue, Color::AliceBlue,
          CurrentConfig->getConfigColor(
              CurrentConfig
                  ->getActiveProfile()["rimcas"]["background_color_stage_one"]),
          CurrentConfig->getConfigColor(
      ...
      if (RimcasLabelColor.ToCOLORREF() !=
          Color(Color::AliceBlue).ToCOLORREF()) {
        RimcasLabelColor =
            ColorManager->get_corrected_color("label", RimcasLabelColor);
        int rimcas_height = 0;
        ...
        graphics.MeasureString(wrimcas_height.c_str(),
                               wcslen(wrimcas_height.c_str()),
                               customFonts[currentFontSize], PointF(0, 0),
                               &Gdiplus::StringFormat(), &RectRimcas_height);
        rimcas_height = int(RectRimcas_height.GetBottom());

        // Drawing the rectangle

        CRect RimcasLabelRect(TagBackgroundRect.left,
                              TagBackgroundRect.top - rimcas_height,
                              TagBackgroundRect.right, TagBackgroundRect.top);
        graphics.FillRectangle(&SolidBrush(RimcasLabelColor),
                               CopyRect(RimcasLabelRect));
        TagBackgroundRect.top -= rimcas_height;
        ...
        graphics.DrawString(
            rimcasw.c_str(), wcslen(rimcasw.c_str()),
            customFonts[currentFontSize],
            PointF(Gdiplus::REAL(
                       (TagBackgroundRect.left + TagBackgroundRect.right) / 2),
                   Gdiplus::REAL(TagBackgroundRect.top)),
            &stformat, &RimcasTextColor);
      }
    }

    // Adding the tag screen object
    tagAreas[rt.GetCallsign()] = TagBackgroundRect;
    AddScreenObject(DRAWING_TAG, rt.GetCallsign(), TagBackgroundRect, true,
    ...
    for (auto &&line : ReplacedLabelLines) {
      ...
      for (auto &&element : line) {
        ...
        if (TagReplacingMap["sqerror"].size() > 0 &&
            strcmp(element.c_str(), TagReplacingMap["sqerror"].c_str()) == 0)
        ...
        graphics.DrawString(
            welement.c_str(), wcslen(welement.c_str()),
            customFonts[currentFontSize],
        ...
        graphics.MeasureString(welement.c_str(), wcslen(welement.c_str()),
                               customFonts[currentFontSize], PointF(0, 0),
                               &Gdiplus::StringFormat(), &mRect);

        CRect ItemRect(
            TagBackgroundRect.left + widthOffset,
        ...
        AddScreenObject(TagClickableMap[element], rt.GetCallsign(), ItemRect,
                        true, GetBottomLine(rt.GetCallsign()).c_str());
...
#pragma endregion Drawing of the tags
  ...
  for (std::map<string, bool>::iterator it =
    ...
    for (auto &Time : TimeDefinition) {
      if (RimcasInstance->TimeTable[it->first].find(Time) !=
          RimcasInstance->TimeTable[it->first].end()) {
        auto acInfo = RimcasInstance->TimeTable[it->first][Time];
        if (detailedRIMCAS) {
          sprintf_s(buffer, "%2d: %-10s %-8s %s", Time, acInfo.callsign.c_str(),
    ...
    int boxPadding =
        detailedRIMCAS ? 15 : 10; // Smaller padding for simple mode
    int boxWidth = maxWidth + boxPadding;
    int boxHeight = LineSpacing * (TimeDefinition.size() + 1) +
                    5; // +1 for runway name, +5px total padding

    if (TimePopupAreas.find(it->first) == TimePopupAreas.end())
      TimePopupAreas[it->first] = {300, 300, 300 + boxWidth, 300 + boxHeight};
    else
      TimePopupAreas[it->first] = {TimePopupAreas[it->first].left,
                                   TimePopupAreas[it->first].top,
                                   TimePopupAreas[it->first].left + boxWidth,
                                   TimePopupAreas[it->first].top + boxHeight};
    ...
    dc.TextOutA(CRectTime.left + CRectTime.Width() / 2 -
                    dc.GetTextExtent(tempS.c_str()).cx / 2,
                CRectTime.top + 2, tempS.c_str());
    ...
    for (auto &Time : TimeDefinition) {
      ...
      if (RimcasInstance->TimeTable[it->first].find(Time) !=
        ...
        if (RimcasInstance->AcColor.find(acInfo.callsign) !=
            RimcasInstance->AcColor.end()) {
          CBrush RimcasBrush(
              RimcasInstance
                  ->GetAircraftColor(
                      acInfo.callsign, Color::Black, Color::Black,
                      CurrentConfig->getConfigColor(
                          CurrentConfig->getActiveProfile()
                              ["rimcas"]["background_color_stage_one"]),
                      CurrentConfig->getConfigColor(
                          CurrentConfig->getActiveProfile()
                              ["rimcas"]["background_color_stage_two"]))
                  .ToCOLORREF());

          CRect TempRect = {CRectTime.left, CRectTime.top + TopOffset,
                            CRectTime.right,
                            CRectTime.top + TopOffset + TextHeight};
        ...
        dc.TextOutA(CRectTime.left + 5, CRectTime.top + TopOffset,
                    tempS.c_str());
      } else {
  ...
  Logger::info("Departure Window: showDepartureWindow=" + std::to_string(showDepartureWindow) + ", DepartedAircraft.size=" + std::to_string(RimcasInstance->DepartedAircraft.size()));
  ...
  if (showDepartureWindow && RimcasInstance->DepartedAircraft.size() > 0) {
    ...
    int totalWidth = 10; // Left padding
    if (depWindowShowCallsign)
      totalWidth += colCallsignWidth + colPadding;
    if (depWindowShowDest)
      totalWidth += colDestWidth + colPadding;
    if (depWindowShowAcType)
    ...
    int boxHeight =
        LineSpacing * (RimcasInstance->DepartedAircraft.size() + 1) +
        10; // +1 for header

    // Update the window area dimensions
    DepartureWindowArea.right = DepartureWindowArea.left + totalWidth;
    DepartureWindowArea.bottom = DepartureWindowArea.top + boxHeight;

    CRect DepWindowRect = DepartureWindowArea;
    DepWindowRect.NormalizeRect();

    // Draw background
    dc.FillRect(DepWindowRect, &BrushGrey);

    // Draw header
    int headerX = DepWindowRect.left + 5;
    int headerY = DepWindowRect.top + 3;
    dc.SetTextColor(RGB(33, 33, 33));

    string headerText = "DEPARTURES";
    dc.TextOutA(DepWindowRect.left + DepWindowRect.Width() / 2 -
    ...
    for (auto &pair : RimcasInstance->DepartedAircraft) {
      ...
      if (depWindowShowAcType) {
        dc.TextOutA(rowX, rowY, info.acType.c_str());
        rowX += colAcTypeWidth + colPadding;
      }

      if (depWindowShowWake) {
        dc.TextOutA(rowX, rowY, info.wakeTurbCat.c_str());
  ...
  if (ShowLists["Conflict Alert ARR"]) {
    GetPlugIn()->OpenPopupList(ListAreas["Conflict Alert ARR"], "CA Arrival",
                               1);
    for (std::map<string, CRimcas::RunwayAreaType>::iterator it =
             RimcasInstance->RunwayAreas.begin();
         it != RimcasInstance->RunwayAreas.end(); ++it) {
      GetPlugIn()->AddPopupListElement(
          it->first.c_str(), "", RIMCAS_CA_ARRIVAL_FUNC, false,
          RimcasInstance->MonitoredRunwayArr[it->first.c_str()]);
    }
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
    ShowLists["Conflict Alert ARR"] = false;
  }

  if (ShowLists["Conflict Alert DEP"]) {
    GetPlugIn()->OpenPopupList(ListAreas["Conflict Alert DEP"], "CA Departure",
                               1);
    for (std::map<string, CRimcas::RunwayAreaType>::iterator it =
             RimcasInstance->RunwayAreas.begin();
         it != RimcasInstance->RunwayAreas.end(); ++it) {
      GetPlugIn()->AddPopupListElement(
          it->first.c_str(), "", RIMCAS_CA_MONITOR_FUNC, false,
  ...
  if (ShowLists["Runway closed"]) {
    ...
    for (std::map<string, CRimcas::RunwayAreaType>::iterator it =
         ...
         it != RimcasInstance->RunwayAreas.end(); ++it) {
      GetPlugIn()->AddPopupListElement(
          it->first.c_str(), "", RIMCAS_CLOSED_RUNWAYS_FUNC, false,
  ...
  if (ShowLists["Visibility"]) {
    ...
    GetPlugIn()->AddPopupListElement("Normal", "", RIMCAS_UPDATE_LVP, false,
                                     int(!isLVP));
    GetPlugIn()->AddPopupListElement("Low", "", RIMCAS_UPDATE_LVP, false,
                                     int(isLVP));
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
  ...
  if (ShowLists["RIMCAS Detail"]) {
    ...
    GetPlugIn()->AddPopupListElement("Detailed", "", RIMCAS_UPDATE_DETAILED,
                                     false, int(detailedRIMCAS));
    GetPlugIn()->AddPopupListElement("Simple", "", RIMCAS_UPDATE_DETAILED,
                                     false, int(!detailedRIMCAS));
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
  ...
  if (ShowLists["Departure Timer"]) {
    GetPlugIn()->OpenPopupList(ListAreas["Departure Timer"], "Departure Timer",
                               1);
    ...
    GetPlugIn()->AddPopupListElement("Show AC Type", "",
                                     RIMCAS_DEP_WINDOW_COL_ACTYPE, false,
                                     int(depWindowShowAcType));
    GetPlugIn()->AddPopupListElement("Show Wake Cat", "",
                                     RIMCAS_DEP_WINDOW_COL_WAKE, false,
                                     int(depWindowShowWake));
    GetPlugIn()->AddPopupListElement("Show QSY Freq", "",
                                     RIMCAS_DEP_WINDOW_COL_FREQ, false,
                                     int(depWindowShowFreq));
    GetPlugIn()->AddPopupListElement("Show Time", "",
                                     RIMCAS_DEP_WINDOW_COL_TIME, false,
                                     int(depWindowShowTime));
    GetPlugIn()->AddPopupListElement("Duration (seconds)", "",
                                     RIMCAS_OPEN_LIST);
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
    ShowLists["Departure Timer"] = false;
  }

  if (ShowLists["Duration (seconds)"]) {
    GetPlugIn()->OpenPopupList(ListAreas["Duration (seconds)"], "Duration", 1);
    GetPlugIn()->AddPopupListElement("60", "", RIMCAS_DEP_WINDOW_DURATION,
                                     false,
                                     int(departureDisplayDuration == 60));
    GetPlugIn()->AddPopupListElement("120", "", RIMCAS_DEP_WINDOW_DURATION,
                                     false,
                                     int(departureDisplayDuration == 120));
    GetPlugIn()->AddPopupListElement("180", "", RIMCAS_DEP_WINDOW_DURATION,
                                     false,
                                     int(departureDisplayDuration == 180));
    GetPlugIn()->AddPopupListElement("240", "", RIMCAS_DEP_WINDOW_DURATION,
                                     false,
                                     int(departureDisplayDuration == 240));
    GetPlugIn()->AddPopupListElement("300", "", RIMCAS_DEP_WINDOW_DURATION,
                                     false,
                                     int(departureDisplayDuration == 300));
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
    ShowLists["Duration (seconds)"] = false;
  }
  ...
  if (ShowLists["Profiles"]) {
    ...
    for (std::vector<string>::iterator it = allProfiles.begin();
         it != allProfiles.end(); ++it) {
      GetPlugIn()->AddPopupListElement(
          it->c_str(), "", RIMCAS_UPDATE_PROFILE, false,
  ...
  if (ShowLists["Colour Settings"]) {
    GetPlugIn()->OpenPopupList(ListAreas["Colour Settings"], "Colour Settings",
                               1);
    GetPlugIn()->AddPopupListElement("Day", "", RIMCAS_UPDATE_BRIGHNESS, false,
                                     int(ColorSettingsDay));
    GetPlugIn()->AddPopupListElement("Night", "", RIMCAS_UPDATE_BRIGHNESS,
                                     false, int(!ColorSettingsDay));
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
  ...
  if (ShowLists["Label Font Size"]) {
    GetPlugIn()->OpenPopupList(ListAreas["Label Font Size"], "Label Font Size",
                               1);
    GetPlugIn()->AddPopupListElement("Size 1", "", RIMCAS_UPDATE_FONTS, false,
                                     int(bool(currentFontSize == 1)));
    GetPlugIn()->AddPopupListElement("Size 2", "", RIMCAS_UPDATE_FONTS, false,
                                     int(bool(currentFontSize == 2)));
    GetPlugIn()->AddPopupListElement("Size 3", "", RIMCAS_UPDATE_FONTS, false,
                                     int(bool(currentFontSize == 3)));
    GetPlugIn()->AddPopupListElement("Size 4", "", RIMCAS_UPDATE_FONTS, false,
                                     int(bool(currentFontSize == 4)));
    GetPlugIn()->AddPopupListElement("Size 5", "", RIMCAS_UPDATE_FONTS, false,
                                     int(bool(currentFontSize == 5)));
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
  ...
  if (ShowLists["GRND Trail Dots"]) {
    GetPlugIn()->OpenPopupList(ListAreas["GRND Trail Dots"], "GRND Trail Dots",
                               1);
    GetPlugIn()->AddPopupListElement("0", "", RIMCAS_UPDATE_GND_TRAIL, false,
                                     int(bool(Trail_Gnd == 0)));
    GetPlugIn()->AddPopupListElement("2", "", RIMCAS_UPDATE_GND_TRAIL, false,
                                     int(bool(Trail_Gnd == 2)));
    GetPlugIn()->AddPopupListElement("4", "", RIMCAS_UPDATE_GND_TRAIL, false,
                                     int(bool(Trail_Gnd == 4)));
    GetPlugIn()->AddPopupListElement("8", "", RIMCAS_UPDATE_GND_TRAIL, false,
                                     int(bool(Trail_Gnd == 8)));
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
  ...
  if (ShowLists["APPR Trail Dots"]) {
    GetPlugIn()->OpenPopupList(ListAreas["APPR Trail Dots"], "APPR Trail Dots",
                               1);
    GetPlugIn()->AddPopupListElement("0", "", RIMCAS_UPDATE_APP_TRAIL, false,
                                     int(bool(Trail_App == 0)));
    GetPlugIn()->AddPopupListElement("4", "", RIMCAS_UPDATE_APP_TRAIL, false,
                                     int(bool(Trail_App == 4)));
    GetPlugIn()->AddPopupListElement("8", "", RIMCAS_UPDATE_APP_TRAIL, false,
                                     int(bool(Trail_App == 8)));
    GetPlugIn()->AddPopupListElement("12", "", RIMCAS_UPDATE_APP_TRAIL, false,
                                     int(bool(Trail_App == 12)));
    GetPlugIn()->AddPopupListElement("16", "", RIMCAS_UPDATE_APP_TRAIL, false,
                                     int(bool(Trail_App == 16)));
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
  ...
  if (ShowLists["Predicted Track Line"]) {
    GetPlugIn()->OpenPopupList(ListAreas["Predicted Track Line"],
                               "Predicted Track Line", 1);
    GetPlugIn()->AddPopupListElement("0", "", RIMCAS_UPDATE_PTL, false,
                                     int(bool(PredictedLength == 0)));
    GetPlugIn()->AddPopupListElement("1", "", RIMCAS_UPDATE_PTL, false,
                                     int(bool(PredictedLength == 1)));
    GetPlugIn()->AddPopupListElement("2", "", RIMCAS_UPDATE_PTL, false,
                                     int(bool(PredictedLength == 2)));
    GetPlugIn()->AddPopupListElement("3", "", RIMCAS_UPDATE_PTL, false,
                                     int(bool(PredictedLength == 3)));
    GetPlugIn()->AddPopupListElement("4", "", RIMCAS_UPDATE_PTL, false,
                                     int(bool(PredictedLength == 4)));
    GetPlugIn()->AddPopupListElement("5", "", RIMCAS_UPDATE_PTL, false,
                                     int(bool(PredictedLength == 5)));
    GetPlugIn()->AddPopupListElement("Close", "", RIMCAS_CLOSE, false, 2, false,
                                     true);
  ...
  if (ShowLists["Label"]) {
    ...
    for (int i = CColorManager::bounds_low(); i <= CColorManager::bounds_high();
         i += 10)
      GetPlugIn()->AddPopupListElement(
          std::to_string(i).c_str(), "", RIMCAS_BRIGHTNESS_LABEL, false,
  ...
  if (ShowLists["Symbol"]) {
    ...
    for (int i = CColorManager::bounds_low(); i <= CColorManager::bounds_high();
         i += 10)
      GetPlugIn()->AddPopupListElement(
          std::to_string(i).c_str(), "", RIMCAS_BRIGHTNESS_SYMBOL, false,
  ...
  if (ShowLists["Afterglow"]) {
    GetPlugIn()->OpenPopupList(ListAreas["Afterglow"], "Afterglow Brightness",
                               1);
    for (int i = CColorManager::bounds_low(); i <= CColorManager::bounds_high();
         i += 10)
      GetPlugIn()->AddPopupListElement(
          std::to_string(i).c_str(), "", RIMCAS_BRIGHTNESS_AFTERGLOW, false,
  ...
  if (QDMenabled || QDMSelectEnabled ||
      (DistanceToolActive && ActiveDistance.first != "")) {
    ...
    if (DistanceToolActive) {
      CPosition r = GetPlugIn()
                        ->RadarTargetSelect(ActiveDistance.first.c_str())
                        .GetPosition()
                        .GetPosition();
  ...
  for (auto &&kv : DistanceTools) {
    ...
    POINT onePoint =
        ConvertCoordFromPositionToPixel(one.GetPosition().GetPosition());
    POINT twoPoint =
        ConvertCoordFromPositionToPixel(two.GetPosition().GetPosition());
    ...
    double Distance = one.GetPosition().GetPosition().DistanceTo(
        two.GetPosition().GetPosition());
    double Bearing = one.GetPosition().GetPosition().DirectionTo(
        two.GetPosition().GetPosition());
    ...
    CRect ClickableRect = {TextPos.x - 2, TextPos.y,
                           TextPos.x + dc.GetTextExtent(text.c_str()).cx + 2,
                           TextPos.y + dc.GetTextExtent(text.c_str()).cy};
    graphics.FillRectangle(&SolidBrush(Color(127, 122, 122)),
                           CopyRect(ClickableRect));
    dc.Draw3dRect(ClickableRect, RGB(75, 75, 75), RGB(45, 45, 45));
    dc.TextOutA(TextPos.x, TextPos.y, text.c_str());

    AddScreenObject(RIMCAS_DISTANCE_TOOL,
  ...
  CRect ToolBarAreaTop(RadarArea.left, RadarArea.top, RadarArea.right,
                       RadarArea.top + 20);
  ...
  AddScreenObject(RIMCAS_ACTIVE_AIRPORT, "ActiveAirport",
                  {ToolBarAreaTop.left + offset, ToolBarAreaTop.top + 4,
                   ToolBarAreaTop.left + offset +
                       dc.GetTextExtent(getActiveAirport().c_str()).cx,
                   ToolBarAreaTop.top + 4 +
                       dc.GetTextExtent(getActiveAirport().c_str()).cy},
  ...
  CRect barDistanceRect = {
      ...
      ToolBarAreaTop.left + offset + dc.GetTextExtent("/").cx,
      ToolBarAreaTop.top + 4 + +dc.GetTextExtent("/").cy};
  if (DistanceToolActive) {
  ...
  for (const auto areas : tagAreas) {
    ...
    if (RecentlyAutoMovedTags.find(areas.first) !=
        RecentlyAutoMovedTags.end()) {
      double t = (double)clock() -
                 RecentlyAutoMovedTags[areas.first] / ((double)CLOCKS_PER_SEC);
      if (t >= 0.8) {
    ...
    POINT acPosPix = ConvertCoordFromPositionToPixel(
        GetPlugIn()
            ->RadarTargetSelect(areas.first.c_str())
            .GetPosition()
            .GetPosition());
    int lenght = LeaderLineDefaultlenght;
    ...
    for (double rotated = 0.0; abs(rotated) <= 360.0;) {
      ...
      CRect NewRectangle(TagCenter.x - (width / 2), TagCenter.y - (height / 2),
                         TagCenter.x + (width / 2), TagCenter.y + (height / 2));
