...
inline static void replaceAll(string &source, const string &from,
                              const string &to) {
  string newString;
...
inline static double TrueBearing(CPosition pos1, CPosition pos2) {
  ...
  double dir = fmod(atan2(sin(lon2 - lon1) * cos(lat2),
                          cos(lat1) * sin(lat2) -
...
inline static int Is_Left(const POINT &p0, const POINT &p1,
                          const POINT &point) {
  return ((p1.x - p0.x) * (point.y - p0.y) - (point.x - p0.x) * (p1.y - p0.y));
};

inline static bool Is_Inside(const POINT &point,
...
//
inline static bool
LiangBarsky(RECT Area, POINT fromSrc, POINT toSrc, POINT &ClipFrom,
            POINT &ClipTo) // The output values, so declare these outside.
{
...
inline static double DegToRad(double x) { return x / 180.0 * M_PI; };

inline static double RadToDeg(double x) { return x / M_PI * 180.0; };

inline static CPosition BetterHarversine(CPosition init, double angle,
                                         double meters) {
  CPosition newPos;
  ...
  double lon =
      cos(lat) == 0
          ? lon0
          : fmod(lon0 + asin(sin(trk) * sin(d) / cos(lat)) + M_PI, 2 * M_PI) -
                M_PI;
...
const int RIMCAS_DEP_WINDOW = 7001;
const int RIMCAS_DEP_WINDOW_ROW = 7002;
const int RIMCAS_DEP_WINDOW_QSY = 7003;
const int RIMCAS_DEP_WINDOW_TOGGLE = 8040;
const int RIMCAS_DEP_WINDOW_DURATION = 8041;
const int RIMCAS_DEP_WINDOW_COL_CALLSIGN = 8042;
const int RIMCAS_DEP_WINDOW_COL_DEST = 8043;
const int RIMCAS_DEP_WINDOW_COL_ACTYPE = 8044;
const int RIMCAS_DEP_WINDOW_COL_WAKE = 8045;
const int RIMCAS_DEP_WINDOW_COL_FREQ = 8046;
const int RIMCAS_DEP_WINDOW_COL_TIME = 8047;
